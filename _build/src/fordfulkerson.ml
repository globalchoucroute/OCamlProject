...
let rec find_path2 gr forbidden id1 id2 = if id1 = id2 then Some [id1] else 
    let rec loop outerarcs forbidden acu currentNode graphend = match outerarcs with
      | [] -> None
      | (id,rest)::tail -> if id = graphend && rest <> 0 then Some (id :: acu) else 
...
let rec fordFulkerson gr id1 id2 = match find_path2 gr [] id1 id2 with
  | None -> gr
  | Some l -> fordFulkerson (incr2 gr l (get_lowest_weight gr l)) id1 id2;; 

(*Algorithme prenant en compte les graphes de flux*)
let run_algorithm gsource source sink=
  let gflow = flowGraphInit gsource in
  let rec loop gf forbidden source sink gr = match find_path2 gr forbidden source sink with
    | None -> gf
    | Some path -> let gf2 = update_path gf path (get_lowest_weight gr path) in
      loop gf2 forbidden source sink (getResidualGraph gf2)
